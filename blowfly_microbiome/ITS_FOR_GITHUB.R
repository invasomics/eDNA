...
setwd("/Users/GGPC/OneDrive/Desktop/microbiome_november/ITS")

asv_mat <- read_tsv("ASVs_counts_ITS_bf.tsv")
tax_mat <- read.csv("MergedTaxonomy_ITS_bf.csv", header = TRUE)
samples_df <- read_excel("metadata_subset_ITS.xlsx")
...
ps_raw   # 4443 taxa, 40 samples

################################################################################
## FILTERING
################################################################################

# Remove uncharacterised taxa
ps_filt <- subset_taxa(
  ps_raw,
...
ps_filt <- subset_taxa(ps_filt, Family != "Hominidae")

# Remove plants
ps_filt <- subset_taxa(
  ps_filt,
  !Phylum %in% c("Streptophyta", "Chlorophyta")
)
...
sample_data(ps_filt)$is.neg <- sample_names(ps_filt) %in% c(
  "blank-ITS-1", "blank-ITS-2", "Blank-ITS-3", "Blank-16S-4",
  "PCRNEG-STEP1-ITS-1", "PCRNEG-STEP1-ITS-3",
  "PCRNEG-STEP2-ITS-2", "PCRNEG-STEP2-ITS-4"
)
...
contaminants <- rownames(contamdf)[contamdf$contaminant]

ps_clean <- prune_taxa(
  !taxa_names(ps_filt) %in% contaminants,
  ps_filt
)
...
################################################################################
## BALANCE SAMPLES (8 PER SPECIES)
################################################################################

sample_df <- as.data.frame(sample_data(ps_clean))
...
# Correct phylum for all Mortierellomycetes entries
tax_table(ps_final)[tax_table(ps_final)[, "Class"] == "Mortierellomycetes", "Phylum"] <- "Mucoromycota"


# Step 1: Filter out unknown/ambiguous classes
...
alpha.test <- generate_alpha_test_single(
  data.obj   = data.obj,
  alpha.obj  = NULL,
  alpha.name = "shannon",
  depth      = NULL,
...
bc.dist <- beta.bc$BC
pcoa_its <- pcoa(bc.dist) #run the pcoa
pcoa_coords <- as.data.frame(pcoa_its$vectors) #extract the coordinates
pcoa_df <- pcoa_coords %>%
...
var1 <- round(pcoa_its$values$Relative_eig[1] * 100, 2)
var2 <- round(pcoa_its$values$Relative_eig[2] * 100, 2)
...
ggplot(pcoa_df, aes(x = Axis.1, y = Axis.2, colour = BLAST_ID)) +
  ...
  coord_fixed(sqrt(pcoa_its$values$Relative_eig[1] /
                     pcoa_its$values$Relative_eig[2])) +
  
  # Axis labels with % variance
  labs(
    x = paste0("Axis 2 (",
               round(100 * pcoa_its$values$Relative_eig[1], 1), "%)"),
    y = paste0("Axis 3 (",
               round(100 * pcoa_its$values$Relative_eig[2], 1), "%)"),
    colour = NULL
  ) +
...
pcoa_its$values$Eigenvalues
round(100 * pcoa_its$values$Relative_eig, 2)
