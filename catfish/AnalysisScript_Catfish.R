...
# Additional correlations with water volume
compositionsV <- read.csv("Vol_correlations.csv")
library("Hmisc")
vol_filter <- rcorr(as.matrix(compositionsV[,1:7], type = "spearman"))
print(vol_filter)
...
first_three <- other_order[1:3]
last_group  <- other_order[-(1:3)]
...
p_other <- ggplot(filter(long_df, scale_group == "Other"), aes(Location, Value)) +
  ...
  facetted_pos_scales(
    y = list(
      Variable %in% first_three ~
        scale_y_continuous(
          limits = c(0, 0.030),
          labels = label_number(accuracy = 0.0001)  # <-- 4 decimal places
        ),
      Variable %in% last_group ~
        scale_y_continuous(
          limits = c(0, 0.0017)
        )
    )
...
# ---- Optional: Save ----
ggsave("Figure3a_May26.jpeg", final_plot, width = 14, height = 10, dpi = 600)
...
# ---- Optional: Save ----
ggsave("Figure3b_May26.jpeg", final_plot, width = 14, height = 10, dpi = 600)
